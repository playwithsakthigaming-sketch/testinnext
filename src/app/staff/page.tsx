...
export default function StaffPage() {
  ...
  const finalRoleOrder = [...orderedRoles, ...otherRoles];
  ...
  const getRoleIcon = (role: string) => {
    ...
    if (role === 'Media Staff' || role === 'Media Editor' || role === 'Media Team') return <Film className="mr-1 h-3 w-3 text-purple-400" />;
    if (role === 'Event Staff' || role === 'Event Organizer' || role === 'Events Team') return <Calendar className="mr-1 h-3 w-3 text-orange-400" />;
    if (role === 'Trainer') return <BookOpen className="mr-1 h-3 w-3 text-cyan-400" />;
    if (role === 'Human Resource Staff' || role === 'Human Resources Team') return <Users className="mr-1 h-3 w-3 text-pink-400" />;
    if (role === 'Marketing Executive') return <Briefcase className="mr-1 h-3 w-3 text-teal-400" />;
     if (role === 'Designing Team') return <Briefcase className="mr-1 h-3 w-3 text-indigo-400" />;
