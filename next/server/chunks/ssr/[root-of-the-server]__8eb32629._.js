...
function Home() {
    const now = new Date();
    const upcomingEvents = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$events$2e$json__$28$json$29$__["default"].events.map((event)=>({
            ...event,
            parsedDate: parseEventDate(event.meetupTime || event.date)
        })).filter((event)=>event.parsedDate && event.parsedDate > now).sort((a, b)=>a.parsedDate.getTime() - b.parsedDate.getTime()).slice(0, 3).map((event)=>{
        const image = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$placeholder$2d$images$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["PlaceHolderImages"].find((img)=>img.id === event.imageId);
        return {
            ...event,
            image
        };
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8eb32629._.js.map