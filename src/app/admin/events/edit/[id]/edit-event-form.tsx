...
import { parseDateTimeString } from "@/lib/date-utils";
...
export function EditEventForm({ event }: { event: EventWithImageUrl }) {
  ...
  const parsedMeetup = parseDateTimeString(event.meetupTime);
  const parsedDeparture = parseDateTimeString(event.departureTime);
