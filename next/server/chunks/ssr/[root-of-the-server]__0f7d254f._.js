...
var { g: global, __dirname } = __turbopack_context__;
{
/* __next_internal_action_entry_do_not_use__ [{"004b3fb709195eed22eac8e785e7059b95b66e3420":"getEvents","006fd5123608df075d1d2146e845af27401e37789e":"syncEventsFromTruckersMP","402871e0af245bde1406e900462418780ab3c85a1b":"deleteEvent","40522eae7f8a25ac6f49e8d0396c7dae36996defb6":"addEventFromTruckersMPUrl"},"",""] */ __turbopack_context__.s({
...
async function addSingleEvent(tmpEvent) {
    const eventsData = await readJsonFile(eventsFilePath);
...
async function addEventFromTruckersMPUrl(eventUrl) {
    ...
    try {
        const res = await fetch(`http://localhost:9002/api/truckersmp?endpoint=events/${eventId}`);
        if (!res.ok) {
...
async function syncEventsFromTruckersMP() {
    try {
        ...
        const tmpData = await res.json();
        const tmpEvents = tmpData.response.events;
        if (!tmpEvents || tmpEvents.length === 0) {
            return {
                success: true,
                message: "No new events found on TruckersMP.",
                added: 0
            };
        }
        const eventsData = await readJsonFile(eventsFilePath);
        ...
        if (addedCount > 0) {
            return {
                success: true,
                message: `Successfully synced ${addedCount} new event(s).`,
                added: addedCount
            };
        }
        return {
            success: true,
            message: 'Events are already up-to-date.',
            added: 0
        };
    } catch (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0f7d254f._.js.map